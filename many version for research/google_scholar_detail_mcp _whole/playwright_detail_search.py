...
OUTPUT_FILENAME = "/Users/hyijie/Desktop/Interest_try_test/test_detail_abstract.xlsx"
MAX_RESULTS = 5
START_PAGE = 0
...
async def is_journal_verification_page(page: Page) -> bool:
    try:
        current_url = page.url.lower()
        if "sorry" in current_url or "verify" in current_url or "challenge" in current_url:
            return True
        ...
        if len(body_text.strip()) < 300:
            ...
            active_captcha_patterns = [
                r'g-recaptcha.*data-sitekey',
                r'h-captcha.*data-sitekey',
            ]
            
            for pattern in active_captcha_patterns:
...
async def wait_for_journal_verification(page: Page, timeout: int = 600):
    print("\n" + "=" * 80)
    print("⚠️  JOURNAL VERIFICATION REQUIRED!")
    print("This journal page requires human verification (CAPTCHA/Cloudflare/etc).")
    print("Please complete the verification in the browser window.")
    print("The script will automatically continue once verification is complete.")
    print("If the page shows a checkbox, please click it to verify.")
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            if not await is_journal_verification_page(page):
                print("✅ Verification completed! Continuing to extract abstract...")
                await asyncio.sleep(2)
                return True
    ...
    print("⚠️  Verification timeout! Skipping this paper...")
    return False
    ...
    try:
        ...
        try:
            ...
            if await is_journal_verification_page(new_page):
                print(f"    ⚠️  VERIFICATION REQUIRED - Waiting for user to complete verification...")
                verification_success = await wait_for_journal_verification(new_page)
                
                if not verification_success:
                    print(f"    SKIPPED - Verification timeout or failed")
                    skipped = True
                    return "", "", True
                
                print(f"    ✅ Verification passed! Now extracting abstract...")
